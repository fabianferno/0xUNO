...
io.on("connection", (socket) => {
  ...
  socket.on("join", async (payload, callback) => {
    ...
    const { error, newUser } = await addUser({
      ...
      name:
        ...
        getUsersInRoom(payload.room)[0].name === "Player 2"
          ? "Player 1"
          : "Player 2",
  ...
  socket.on("initGameState", async (gameState, address) => {
    const user = await getUserWithAddress(address);
    if (user) io.to(user.room).emit("initGameState", gameState);
  });

  socket.on("updateGameState", async (gameState, address) => {
    const user = await getUserWithAddress(address);
    if (user) {
  ...
  socket.on("sendMessage", async (payload, callback) => {
    const user = await getUserWithAddress(payload.address);
